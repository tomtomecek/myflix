...
["Interstellar", "V for Vendetta", "Inception", "District 9", "Family guy", "Monk", 
  ...
  video.small_cover.store!(File.open(File.join(Rails.root, "public/tmp/#{title.gsub(' ', '_').downcase}.jpg")))
  video.large_cover.store!(File.open(File.join(Rails.root, "public/tmp/#{title.gsub(' ', '_').downcase}_large.jpg")))
  video.save!
